...
class RoleHashtagBox extends ConsumerStatefulWidget {
  const RoleHashtagBox({
    super.key,
    required this.title,
    this.hintText,
  ...
  @override
  ConsumerState<RoleHashtagBox> createState() => _GenreHashtagBoxState();
}

class _GenreHashtagBoxState extends ConsumerState<RoleHashtagBox> {
  Timer? _debounce;
  ...
  void onChanged(String value) {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () {
      ...
      if (value.contains(' ')) query = value.split(' ').last;

      ref.read(skillProvider.notifier).search(query.replaceFirst('#', ''));
    });
  }
}
...
class HashtagAutocompleteOptions extends ConsumerWidget {
  ...
  final String query;
  final ValueSetter<SkillOut> onHashtagTap;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    var hashtag = ref.watch(skillProvider).valueOrNull?.items ?? [];
